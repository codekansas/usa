...
@ml.register_model("ms_point2emb", Point2EmbModelConfig)
class Point2EmbModel(ml.BaseModel[Point2EmbModelConfig]):
    def __init__(self, config: Point2EmbModelConfig) -> None:
        ...
        clip_layers += [
            nn.Sequential(
                nn.Linear(
                    pos_mlp_in_dims + 1 if i == 0 else config.hidden_dims,
        ...
        self.clip_position_mlp = nn.Sequential(*clip_layers)

        # SDF prediction is not related to scale
        sdf_layers: list[nn.Module] = []
        sdf_layers += [
            nn.Sequential(
                nn.Linear(
                    pos_mlp_in_dims if i == 0 else config.hidden_dims,
                    1 if i == config.num_layers - 1 else config.hidden_dims,
                ),
                ml.get_norm_linear(
                    "no_norm" if i == config.num_layers - 1 else norm_type,
                    dim=1 if i == config.num_layers - 1 else config.hidden_dims,
                ),
                ml.get_activation(
                    "no_act" if i == config.num_layers - 1 else act_type,
                    inplace=True,
        ...
        self.clip_position_mlp = nn.Sequential(*clip_layers)
        self.sdf_position_mlp = nn.Sequential(*sdf_layers)

        self.apply(init_weights)

    def forward(self, points: Tensor, scales = 0.5) -> Tensor:
        ...
        if type(scales) == type(0):
            scales = scales * torch.ones(*points.shape[:-1], 1).to(points)
        pos_embs = self.pos_embs(points)
        scale_pos_embs = torch.cat((pos_embs, scales.view(*points.shape[:-1], -1)), dim = -1)
        sdf_preds = self.sdf_position_mlp(pos_embs)
        clip_preds = self.clip_position_mlp(scale_pos_embs)

        return torch.cat([clip_preds, sdf_preds], dim = -1)
